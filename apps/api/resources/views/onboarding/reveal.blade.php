<x-app-layout>
    ...
    @php
        $portfolioValue = (float) data_get($summary, 'portfolio_value', 0);
        $accountCount = (int) data_get($summary, 'account_count', 0);
        $holdingCount = (int) data_get($summary, 'holding_count', 0);
        $transactionCount = (int) data_get($summary, 'transaction_count', 0);
        $connectedAccounts = collect(
            data_get($summary, 'connected_accounts', [])
        );
    @endphp
        ...
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <section
                x-data="{
                    ...
                    animateValue(key, duration = 1400) {
                    ...
                    init() {
                        window.setTimeout(() => {
                            this.started = true;
                            this.animateValue('portfolioValue', 1800);
                        }, 250);

                        window.setTimeout(() => {
                            this.showMetrics = true;
                            this.animateValue('accountCount', 800);
                            this.animateValue('holdingCount', 1000);
                            this.animateValue('transactionCount', 1200);
                        }, 900);
            ...
            >
                <div class="relative overflow-hidden px-6 py-8 sm:px-10 sm:py-12 lg:px-14 lg:py-14">
                    ...
                    <div class="pointer-events-none absolute -bottom-28 -left-28 h-80 w-80 rounded-full bg-cyan-400/10 blur-3xl"></div>
                    ...
                    <div class="relative">
                        <div
                            ...
                            class="text-center"
                        >
                            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-400/10 text-emerald-300 ring-1 ring-emerald-300/20">
                            ...
                            <p class="mx-auto mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
                                Helmio successfully reviewed every connected account,
                                holding, and transaction currently available.
                            </p>
                        </div>
                        ...
                        >
                            ...
                            <p
                                class="mt-3 break-words text-5xl font-bold tracking-tight text-white sm:text-7xl"
                        ...
                        >
                            <div class="rounded-2xl border border-white/10 bg-white/10 p-5 text-center">
                                <p class="text-3xl font-bold" x-text="formatNumber(accountCount)"></p>
                                <p class="mt-2 text-sm text-slate-300">Connected accounts</p>
                            </div>

                            <div class="rounded-2xl border border-white/10 bg-white/10 p-5 text-center">
                                <p class="text-3xl font-bold" x-text="formatNumber(holdingCount)"></p>
                                <p class="mt-2 text-sm text-slate-300">Holdings reviewed</p>
                            </div>

                            <div class="rounded-2xl border border-white/10 bg-white/10 p-5 text-center">
                                <p class="text-3xl font-bold" x-text="formatNumber(transactionCount)"></p>
                                <p class="mt-2 text-sm text-slate-300">Transactions reviewed</p>
                            </div>
                        </div>
                            ...
                            >
                                ...
                                <div class="mt-4 space-y-3">
                                    @foreach ($connectedAccounts->take(5) as $account)
                                        <div class="flex min-w-0 items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
                                            <div class="flex min-w-0 items-center gap-3">
                                                ...
                                                <div class="min-w-0">
                                                    ...
                                                    <p class="mt-1 truncate text-sm text-slate-400">
                                                        {{ data_get($account, 'institution', 'Connected Institution') }}
                                                    </p>
                                                </div>
                                            </div>

                                            <p class="shrink-0 text-sm font-semibold text-slate-200">
                                                {{ money(data_get($account, 'value', 0)) }}
                                            </p>
                                        </div>
                                    @endforeach
                                </div>
