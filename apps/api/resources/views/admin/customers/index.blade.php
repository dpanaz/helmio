...
<tr class="hover:bg-slate-800/40"><td class="px-5 py-4"><p class="font-semibold text-white">{{ $customer->name }}</p><p class="mt-1 text-sm text-slate-500">{{ $customer->email }}</p></td><td class="px-5 py-4 text-sm">{{ $customer->investment_accounts_count }}</td><td class="px-5 py-4 text-sm">{{ $customer->brokerage_connections_count }}</td><td class="px-5 py-4 text-sm">{{ $customer->ask_helmio_conversations_count }}</td><td class="px-5 py-4 text-sm text-slate-400">{{ $customer->created_at?->format('M j, Y') }}</td><td class="px-5 py-4 text-right"><a href="{{ route('admin.customers.show', $customer) }}" class="font-semibold text-blue-400">Open</a></td></tr>
@empty
